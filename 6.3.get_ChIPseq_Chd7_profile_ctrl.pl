...
open(IND,"Random_1000_control_genes.txt")||die("Can't open IND:$!\n");
 
print "Please enter the bg file name: no need the '.bg'\n";
chomp($name=<STDIN>);

open(INC,"Chd7/$name.bg")||die("Can't open INA:$!\n");
open(OUT,">Chd7/heatmap/Heatmap_ctrl_$name.txt")||die("Can't open INA:$!\n");
...
{
 chomp;
 if(/^(ENSMUSG\d+\_[\w\.\-]+)$/){chomp; $a1++; $deg{$1}=1;}
 else{print "Error1\t$_\n";}
}
