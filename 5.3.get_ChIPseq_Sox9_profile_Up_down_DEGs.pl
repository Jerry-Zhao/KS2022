...
while(<INA>)
{
 ...
 if(/^(ENSMUSG\d+\_[\w\.\-]+)\t([\w\.\-]+)\t[\w\.\-]+\t([\w\.\-]+)\t[\w\.\-]+$/)
 {
  chomp;
  $a1++;
  if($2>0 and $3>0) {$a2++; $deg{$1}=1;}
  if($2<0 and $3<0) {$a3++; $deg{$1}=1;}
 }
 else{print "Error1\t$_\n";}
}
...
print "Overlap-DEG: $a1\n";
print "Both-up: $a2\t Both-down: $a3\n";
print "Both-up or Both-down: $b23 $c23\n";
